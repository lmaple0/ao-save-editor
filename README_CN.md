...
- 当前 96 项单元测试全部通过。用户标注的 8 个队伍样本与原始槽位顺序逐项一致；122 个有效本机存档均无未知队伍/外观 ID，且只读验证前后哈希不变；128 个有效本机存档的元素限定检查为零冲突，Windows Python 3.13 隐藏窗口测试确认限定槽只显示兼容回路。
...
- `ao_monster_details.json`：由 `build_monster_details.py` 使用独立、带边界检查的只读解析器从本机三语 333 个 `ms*.dat` 生成；保留每个来源文件的 SHA-256，不执行或分发上游生成脚本及原始游戏文件。
- `ao_reference_graph.json`：由 `build_reference_graph.py` 连接怪物目录、三语 `t_name._dt`、333 个 `ms` 与三语有效 `as` 动作表；保留相对来源路径和 SHA-256。动作入口按索引关联，不能证明属于本地脚本的战技引用只列为未解析研究项。
