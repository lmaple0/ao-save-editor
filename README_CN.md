...
- 启动时通过 Windows Saved Games 系统目录自动定位 `FALCOM/Ao`，批量识别 `dataNNNN/savedata.dat`；左侧列表显示槽位、完整校验状态、修改时间和大小，可刷新、切换目录并加载选中存档。切换时会提示尚未保存的界面修改可能丢失，失败加载不会覆盖当前存档对象。存档侧栏可一键收起；中英日标签页使用紧凑专用标题，避免高 DPI 下被压缩截断。
