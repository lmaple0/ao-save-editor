...
- Browse and search a 305-entry monster catalog by localized name, code, file, or location; filter through 75 detailed locations; diagnose unregistered/partial/full records; set selected monsters to full data or unregistered; and unlock all known records. The editor switches monster names and locations with the global Chinese/English/Japanese UI language: CLE Chinese comes from `data_cn`, Japanese from `data`, and NISA English from `data/battle_us` and `data/text_us`. Ouroboros/Falcom `t_dbmon.py` remains the identity, level, and active/commented provenance layer.
...
- The chest generator verifies all 280 flags, 44 localized maps, and 93 NISA scenario files. Two local NISA sample saves both report 117/280 obtained chests. All 29 unit tests pass, and the chest feature never writes save flags.
