...
- `ao_reference_graph.json`: generated by `build_reference_graph.py` by joining the monster catalog, localized `t_name._dt`, 333 `ms` files, and effective localized `as` action tables. It keeps relative provenance paths and SHA-256 values; craft references that cannot be proven local remain unresolved research records.
